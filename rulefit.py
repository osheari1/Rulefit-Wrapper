...
class RuleFit(object):
    ...
    def _load_r_variable_importance_objects(self):
        ...
        index = [self.data['x'].columns.values[int(var-1)] for var in
                    list(robjects.globalenv['var_imp'][1])]
    ...
    def _generate_interaction_null_models(self, n, quiet):
        """ Generates bootstrapped null interaction models to calibrate 
            interaction effects. See FP 2004 8.3. This will create
            a global object in the R instance. This function needs to be called
            before calling interaction functions.
            Args:
    ...
    def generate_interaction_effects(self, nval=10, n=10, quiet=False):
        """ Loads R variable interaction effect objects
            Args:
                ...
                n -> Number of null models to generate for interaction
                     calibaration
        ...
        int_str = """
                    function(ncols, nval){
                        if(exists("null.models")){
                            ...
                            interactions <- interact(c(1:ncols), nval=nval,
                                                    plot=F)
                         }
                    }
                  """
    ...
    def fit(self, x, y, wt=None, cat_vars=None, not_used=None,
        ...
        """ Fit rulefit model. This function will populate a series of RuleFit 
            properties in this class
            Args:
                x -             Pandas dataframe of training data. 
                y -             Input response values. For classification values 
                                must be only 1 or -1. If y is a single scalar, it 
                ...
                test_reps -     number of CV replications used for model selection.
                                ...
                                >0 - test_reps fold cross validation. Final model
                                     ...
                                     4 * fpos * (1 - fpos), fpos = total +1 labels
                                     / total labels
                                Default: test_reps = round(min(20, max(0.0, 5200 / neff - 2)))
                ...
                samp_fract -    Fraction of randomly chosen training obs used to
                                ...
                                Default: samp_fract=min(1, (11*sqrt(neff)+1)/neff)
                inter_supp -    Insentive factor for using fewer vars in tree based
...
def main():
    ...
    boston = pd.read_csv('/home/riley/Programming/PythonProjects/rulefit/boston.csv',
                         index_col=False)
    boston['target'] = np.select([boston.medv > boston.medv.quantile(0.5)],
                                 [1], [-1])
    

    model = RuleFit('linux', '/home/riley/R/lib/rulefit')
    model.fit(x=boston.drop(['medv', 'target'], axis=1),
              y=boston['target'],
              rfmode='class', tree_size=5, mod_sel=3)
    
    model.generate_interaction_effects(nval=10, n=1, quiet=False)
